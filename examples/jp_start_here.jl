...
function pooley_sis_model()
    ...
    ## define model
    return DiscuitModel("SIS", ic, sis_rf, m_t, obs_fn, weak_prior, si_gaussian, 0)
end
...
function mcmc_example()
    ...
    model = pooley_sis_model()
    rs = run_single_chain_analysis(model, obs, [0.003, 0.1])
    print_results(rs, "./out/mcmc_example/")
end
